...
output "public_subnet_id" {
  ...
  value       = try(data.aws_subnet.public[0].id, "")
}

output "security_group_id" {
  description = "ID of the discovered security group"
  value       = try(data.aws_security_group.main[0].id, "")
}

output "ami_id" {
  description = "ID of the discovered AMI"
  value       = try(data.aws_ami.amazon_linux[0].id, "")
}
...
output "instance_public_ip" {
  ...
  value       = try(aws_instance.main[0].public_ip, "")
}
