...
resource "aws_security_group_rule" "public_http_ingress_http" {
  ...
  protocol          = "tcp"
  cidr_blocks       = var.allowed_ip_range
  security_group_id = aws_security_group.public_http[0].id
}
...
resource "aws_security_group_rule" "private_http_ingress_http_from_public_http_sg" {
  ...
  to_port                  = 8080
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.public_http[0].id
  security_group_id        = aws_security_group.private_http[0].id
}
...
resource "aws_network_interface_sg_attachment" "public_instance_ssh" {
  ...
  security_group_id    = aws_security_group.ssh[0].id
  network_interface_id = data.aws_network_interface.public_primary[0].id
}
...
resource "aws_network_interface_sg_attachment" "private_instance_ssh" {
  ...
  security_group_id    = aws_security_group.ssh[0].id
  network_interface_id = data.aws_network_interface.private_primary[0].id
}
